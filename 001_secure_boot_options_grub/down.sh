...
for param_value in "${kernel_params[@]}"; do
    ...
    if grubby --info=ALL | grep -q "$param_value"; then
        ...
        grubby --update-kernel=ALL --remove-args="$param_value"
        echo "Removed parameter '$param_value' to kernel command line."
    else
        echo "Parameter '$param_value' doesn't exist. No action taken."
    fi
done
